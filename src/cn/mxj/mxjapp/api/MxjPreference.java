...
/** 存储系统设置 */
public class MxjPreference {

	private static final String PREFERENCE_NAME = "preference.db";

	private static SharedPreferences mPreference;
	private static MxjPreference mxjPreference; 

	private MxjPreference(Context ctx) {
		mPreference = ctx.getApplicationContext().getSharedPreferences(PREFERENCE_NAME, Context.MODE_PRIVATE);
	}
	
	public static MxjPreference getInstance(Context context){		
		if(mxjPreference == null){
			mxjPreference = new MxjPreference(context);
		}
		return mxjPreference;
	}
